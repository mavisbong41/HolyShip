  ...
  return <div className="detail-section reply-email-section" aria-label="Reply Email">
    ...
    <div className="reply-email-card">
      ...
      {sent ? <div className="reply-success" role="status"><Check size={14} /> Reply sent</div> : <>
        {state === "NOT_AVAILABLE" ? <button type="button" className="button-secondary btn-compact reply-action-button" disabled>Generate Reply</button> : <button type="button" className="button-secondary btn-compact reply-action-button" disabled={busy !== null} onClick={() => void generate()}>{busy === "generate" ? "Generating…" : requestInfo ? "Generate Request Email" : "Generate Reply"}</button>}
        {draft && <div className="reply-draft-box"><span className="reply-draft-label">{requestInfo ? "Request for information" : "Resolution reply"}</span><label>Subject<input aria-label="Reply subject" value={subject} onChange={(event) => setSubject(event.target.value)} /></label><label>Email body<textarea aria-label="Reply draft" value={draft} onChange={(event) => setDraft(event.target.value)} rows={7} /></label><div className="reply-refine-row"><input aria-label="Refine instruction" placeholder="Optional refine instruction" value={refinement} onChange={(event) => setRefinement(event.target.value)} /><button type="button" className="button-secondary btn-compact" disabled={busy !== null || !refinement.trim()} onClick={() => void refine()}>{busy === "refine" ? "Refining…" : "Refine"}</button></div><button type="button" className="button-secondary btn-compact reply-action-button" disabled={busy !== null || !draft.trim()} onClick={() => void send()}><Send size={14} />{busy === "send" ? "Sending…" : confirmSend ? "Confirm & Send" : "Send Reply"}</button></div>}
      </>}
