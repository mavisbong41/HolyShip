...
_SI_REFERENCE = re.compile(r"\b(si|shipping\s+instructions?)\b", re.I)
_BL_REFERENCE = re.compile(r"\b(draft\s+bl|draft\s+bill\s+of\s+lading)\b", re.I)
...
def evaluate_readiness(email: ClassificationInput, category: str) -> str | None:
    ...
    # The sender is asking for an immediate comparison and identifies both
    # required documents as the current inputs. Missing bytes/attachments are
    # therefore a retrieval-stage failure, not uncertain business readiness.
    if _CHECKING.search(text) and _SI_REFERENCE.search(text) and _BL_REFERENCE.search(text):
        return READY_FOR_COMPARISON
    return UNRESOLVED
